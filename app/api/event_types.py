...
def get_all_event_types(session: Session = Depends(get_db_session)):
    """Get all event types"""
    return session.exec(select(EventType)).all()

@router.get("/{id}", response_model=EventType)
def get_single_event_type(id: UUID, session: Session = Depends(get_db_session)):
...
def post_event_type(event_type: EventTypeCreate, session: Session = Depends(get_db_session)):
...
def update_event_type(id: UUID, payload: EventTypeUpdate, session: Session = Depends(get_db_session)):
...
@router.delete("/{id}")
def delete_event_type(id: UUID, session: Session = Depends(get_db_session)):
