...
from app.services.queries import get_user_for_user_roles, get_role_for_user_roles, get_user_role
...
async def update_user_role(user_id: UUID, role_id: UUID, payload: UserRoleUpdate, session: Session = Depends(get_db_session)):
    ...
    user_role = get_user_role(session, user_id, role_id)
    ...
    try:
        # no need to add the user role again, it's already in the session from get_user_role
        session.commit()
