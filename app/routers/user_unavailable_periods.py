...
@router.get("/user_unavailable_periods", response_model=list[UserUnavailablePeriodPublic])
async def get_user_unavailable_periods(session: Session = Depends(get_db_session)):
    user_unavailable_periods = session.exec(
    ...
    return [
        ...
        for upa in user_unavailable_periods
    ]

# # Get single user unavailable period
# @router.get("/users/{user_id}/dates/{date}", response_model=UserDateOut)
# async def get_user_unavailable_period(user_id: UUID, date: datetime.date, conn: asyncpg.Connection = Depends(get_db_connection)):
#     return await fetch_one(conn, table="user_unavailable_periods", filters={"user_id": user_id, "date": date})
