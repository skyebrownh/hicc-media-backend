...
async def get_team(id: str, pool=Depends(get_db_pool)):
    async with pool.acquire() as conn:
        return await fetch_one(conn, table="teams", filters={"team_id": id})
...
async def patch_team(id: str, team_update: TeamUpdate, pool=Depends(get_db_pool)):
    async with pool.acquire() as conn:
        return await update_team(conn, team_id=id, payload=team_update)

@router.delete("/{id}", response_model=TeamOut)
async def delete_team(id: str, pool=Depends(get_db_pool)):
    async with pool.acquire() as conn:
        return await delete_one(conn, table="teams", filters={"team_id": id})