...
async def get_schedule_date_roles(conn: asyncpg.Connection = Depends(get_db_connection)):
...
async def get_schedule_date_role(schedule_date_role_id: UUID, conn: asyncpg.Connection = Depends(get_db_connection)):
...
async def post_schedule_date_role(schedule_date_role: ScheduleDateRoleCreate, conn: asyncpg.Connection = Depends(get_db_connection)):
...
async def delete_schedule_date_role(schedule_date_role_id: UUID, conn: asyncpg.Connection = Depends(get_db_connection)):
