...
# - /user_roles for direct collection operations (POST)
# - /users/{user_id}/roles for user-centric operations (GET, POST all roles)
# - /roles/{role_id}/users for role-centric operations (GET, POST all users)
# - /users/{user_id}/roles/{role_id} for specific resource operations (GET, PATCH, DELETE)
router = APIRouter()

# Get all roles for a user
@router.get("/users/{user_id}/roles", response_model=list[UserRoleOut])
async def get_roles_for_user(user_id: UUID, conn: asyncpg.Connection = Depends(get_db_connection)):
...
async def get_users_for_role(role_id: UUID, conn: asyncpg.Connection = Depends(get_db_connection)):
