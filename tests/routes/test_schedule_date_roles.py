...
from tests.utils.constants import (
    BAD_ID_0000, SCHEDULE_DATE_ID_1, SCHEDULE_DATE_ID_2, MEDIA_ROLE_ID_1, MEDIA_ROLE_ID_2, MEDIA_ROLE_ID_3,
    USER_ID_1, USER_ID_2, SCHEDULE_DATE_ROLE_ID_1, SCHEDULE_DATE_ROLE_ID_2
)
...
async def test_get_all_schedule_date_roles_success(async_client, seed_dates, seed_users, seed_schedules, seed_schedule_date_types, seed_schedule_dates, seed_media_roles, seed_schedule_date_roles, test_dates_data, test_users_data, test_schedules_data, test_schedule_date_types_data, test_schedule_dates_data, test_media_roles_data, test_schedule_date_roles_data):
    ...
    # DATE_2025_05_01/02 (indices 3, 4)
...
async def test_get_single_schedule_date_role_success(async_client, seed_dates, seed_schedules, seed_schedule_date_types, seed_schedule_dates, seed_media_roles, seed_schedule_date_roles, seed_users, test_dates_data, test_schedules_data, test_schedule_date_types_data, test_schedule_dates_data, test_media_roles_data, test_schedule_date_roles_data, test_users_data):
    ...
    response = await async_client.get(f"/schedule_date_roles/{SCHEDULE_DATE_ROLE_ID_1}")
    assert response.status_code == status.HTTP_200_OK
    response_json = response.json()
    assert isinstance(response_json, dict)
    assert response_json["schedule_date_role_id"] == SCHEDULE_DATE_ROLE_ID_1
...
@pytest.mark.parametrize("date_indices, schedule_indices, type_indices, schedule_date_indices, media_role_indices, schedule_date_role_indices, payload, expected_status", [
    ...
    # Duplicate schedule_date_role (date_index 4 = May date, schedule_date_index 1 = second schedule_date)
    # Seed schedule_date_role on SCHEDULE_DATE_ID_2, then try to insert duplicate
    ([3, 4], [1], [0], [1], [0], [2], {"schedule_date_id": SCHEDULE_DATE_ID_2, "media_role_id": MEDIA_ROLE_ID_1}, status.HTTP_409_CONFLICT),
    ...
    ([3], [1], [0], [0], [], [], {"schedule_date_id": SCHEDULE_DATE_ID_1, "media_role_id": BAD_ID_0000}, status.HTTP_404_NOT_FOUND),
    # Foreign key violation (user doesn't exist)
    ([3], [1], [0], [0], [1], [], {"schedule_date_id": SCHEDULE_DATE_ID_1, "media_role_id": MEDIA_ROLE_ID_2, "assigned_user_id": BAD_ID_0000}, status.HTTP_404_NOT_FOUND),
])
@pytest.mark.asyncio
